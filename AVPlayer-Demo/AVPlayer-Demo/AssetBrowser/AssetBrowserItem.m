...
- (id)initWithURL:(NSURL *)URL title:(NSString *)title {
    if ((self = [super init])) {
        ...
        _haveRichestTitle = title != nil;
        _title = title ? [title copy] : [[URL lastPathComponent] stringByDeletingPathExtension];

        // Assume we can generate a thumb unless we have loaded the assets or tried already and know otherwise.
        _canGenerateThumbnails = YES;
...
- (id)initWithAssetItem:(AssetBrowserItem *)browserItem {
    if ((self = [super init])) {
        ...
        _URL = browserItem.URL;

        // May have been an initialization time property.
        _title = browserItem.title;
        _haveRichestTitle = browserItem.haveRichestTitle;

        _thumbnailImage = browserItem.thumbnailImage;
        asset = [browserItem copyAssetIfCreated];

        _canGenerateThumbnails = browserItem.canGenerateThumbnails;
        _audioOnly = browserItem.audioOnly;
...
- (AVAsset *)getAssetInternal {
    ...
    if (asset == nil) {
        asset = [[AVURLAsset alloc] initWithURL:_URL options:nil];
    }
    return asset;
}
...
- (void)generateTitleFromMetadataAsynchronouslyWithCompletionHandler:(void (^)(NSString *title))handler {
    ...
    dispatch_async(assetQueue, ^(void) {
        ...
        [titleAsset loadValuesAsynchronouslyForKeys:@[@"commonMetadata"] completionHandler:^{
            dispatch_async(assetQueue, ^(void) {
                NSString *title = nil;
...
CGRect makeRectWithAspectRatioOutsideRect(CGSize aspectRatio, CGRect containerRect) {
    ...
    CGFloat maxScale = (CGFloat) fmax(scale.width, scale.height);
...
- (CGSize)maxSizeForImageGeneratorToCropAsset:(AVAsset *)thumbnailAsset toSize:(CGSize)size {
    ...
    if (([videoTracks count] > 0)) {
        ...
        if ([formatDescriptions count] > 0) {
            CMVideoFormatDescriptionRef videoFormatDescription = (__bridge CMFormatDescriptionRef) formatDescriptions[0];
            ...
            naturalSizeTransformed.width = (CGFloat) fabs(naturalSizeTransformed.width);
            naturalSizeTransformed.height = (CGFloat) fabs(naturalSizeTransformed.height);
        }
        else {
            return CGSizeZero;
        }
...
- (UIImage *)copyImageFromCGImage:(CGImageRef)image croppedToSize:(CGSize)size {
    ...
    cropRect.origin.x = (CGFloat) round(cropRect.origin.x);
    cropRect.origin.y = (CGFloat) round(cropRect.origin.y);
...
- (void)generateThumbnailFromAsset:(AVAsset *)thumbnailAsset withSize:(CGSize)size fillMode:(AssetBrowserItemFillMode)mode completionHandler:(void (^)(UIImage *thumbnail))handler {
    ...
    [imageGenerator generateCGImagesAsynchronouslyForTimes:@[imageTimeValue] completionHandler:
...
- (void)generateThumbnailAsynchronouslyWithSize:(CGSize)size fillMode:(AssetBrowserItemFillMode)mode completionHandler:(void (^)(UIImage *thumbnail))handler {
    dispatch_async(assetQueue, ^(void) {
        ...
        [thumbnailAsset loadValuesAsynchronouslyForKeys:@[@"tracks"] completionHandler:^(void) {
            dispatch_async(assetQueue, ^(void) {
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<AssetBrowserItem: %p, '%@'>", (__bridge void *) self, self.title];
}
